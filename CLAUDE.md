...
- **Equity history** — `equity_history` is one row per NYSE trading day (`snapshot_date` unique), written by `record_snapshot()` at the **top of `run_eod_review()`, before its no-session early return**, so it lands on every trading day. Read via `GET /api/v1/portfolio/equity-history?days=90` (oldest first). `record_snapshot()` swallows all its own errors and returns `None` — telemetry must never break Phase 3. `broker_equity` is NULL outside `alpaca_*` modes (PaperBroker reports cash as equity, which would be misleading stored next to `total_value`). **This exists because `portfolio` is a single row updated in place and reconstructing history from `trade_history` is unsafe** — see `.handovers/2026-08-06-performance-review.md` §2.
